...
#define TOUCH_WORKMODE TOUCH_WORKMODE_CT
//---------------filter-----------------
...
//--------------lib param-----------------
#if TOUCH_WORKMODE == TOUCH_WORKMODE_CS
#define TKY_BUFLEN  			28
#elif TOUCH_WORKMODE == TOUCH_WORKMODE_CT
#define TKY_BUFLEN  			88
#endif
//---------------------------------------
typedef struct
{
...
typedef struct
{
        ...
        int8_t baseDiffCount;    
        uint8_t baseUpCount;     
        uint16_t deBounceCount;  
        uint16_t rfu;            
#if TOUCH_WORKMODE == TOUCH_WORKMODE_CT
        union {
            uint16_t chargeTime[4];
            uint16_t CTransN[4];
        };

        union {
            uint16_t dischargeTime[4];
            uint16_t CTransCC[4];
        };

        uint16_t baseLine[4];    
        uint16_t curChData[4];   
        int32_t chValue[4];      
        uint32_t threshold[4];   
        uint32_t threshold2[4];  
#elif TOUCH_WORKMODE == TOUCH_WORKMODE_CS
        union {
            uint16_t chargeTime[1];
            uint16_t CTransN[1];
        };

        union {
            uint16_t dischargeTime[1];
            uint16_t CTransCC[1];
        };

        uint16_t baseLine[1];    
        uint16_t curChData[1];   
        int32_t chValue[1];      
        uint32_t threshold[1];   
        uint32_t threshold2[1];  
#endif
} t_touchKeyBuf;
