...
// #define TOUCH_WORKMODE_CS 0
// #define TOUCH_WORKMODE_CT 1

// #define TOUCH_WORKMODE TOUCH_WORKMODE_CT  // TOUCH_WORKMODE_CS
// #define TOUCH_WORKMODE TKEY_WorkMode_ChargeTransfer  // TKEY_WorkMode_CurrentSourceCharge

// #define TOUCH_TEST_SCAN  // Multi-channel scan conversion
#define TOUCH_TEST_SINGLE  // Single-channel single conversion
...
uint8_t cctrans[4] = {0,1,3,6};     //T2_T4
...
static void touch_Regcfg (void) {
    ...
    ADC1->CTLR2 |= (1 << 25) | (1 << 20);  // Trigger TKY conversion
    ADC_Cmd (ADC1, ENABLE);
    ...
    touch_GPIOModeCfg (GPIO_Mode_AIN, 1); 
    TKEY_ChargeTransferChannelConfig (TKEY1, TKEY_Channel_None, TKEY_Channel_1);
...
static void touch_Channelinit (void) {
    ...
    for (uint8_t i = 0; i < TKY_MAX_QUEUE_NUM; i++) {
        ...
        chx_mean = TKY_GetCurChannelMean (my_tky_ch_init[i].channelNum, my_tky_ch_init[i].CTransN,
                                          my_tky_ch_init[i].CTransCC, 10);

        baselineup = 2100;
        baselinedown = 2048;
        TKY_SetGroupCurQueueThreshold(1,i,my_tky_ch_init[i].threshold,my_tky_ch_init[i].threshold2);
        TKY_SetGroupCurQueueThreshold(2,i,my_tky_ch_init[i].threshold,my_tky_ch_init[i].threshold2);
        TKY_SetGroupCurQueueThreshold(3,i,my_tky_ch_init[i].threshold,my_tky_ch_init[i].threshold2);
    ...
    if (error_flag != 0) {
        ...
        for ( j = 0; j < 4; j++) {
            ...
            for (uint8_t i = 0; i < TKY_MAX_QUEUE_NUM; i++) { 
                ...
                chx_mean = TKY_GetCurChannelMean (my_tky_ch_init[i].channelNum, charge_time_CTransN, cctrans[j], 10);  // 

                chx_mean = TKY_GetCurChannelMean (my_tky_ch_init[i].channelNum, charge_time_CTransN, my_tky_ch_init[i].CTransCC, 10);  // cctrans[j]
                    ...
                    if ((charge_time_CTransN == 0) && ((chx_mean > baselineup))) {
                        ...
                        if ((chx_mean > baselinedown) && (chx_mean < baselineup)) {  // Charging parameters are normal
                            TKY_SetGroupCurQueueBaseLine (j,i, chx_mean);
                            TKY_SetGroupCurQueueChargeTime (j,i, charge_time_CTransN, cctrans[j]);

                            dg_log ("channel:%u, chargetime:%u,BaseLine:%u\r\n",
                                    i, charge_time_CTransN, chx_mean);
                            break;
                        } else if (chx_mean >= baselineup) {
                            TKY_SetGroupCurQueueBaseLine (j,i, chx_mean_last);
                            TKY_SetGroupCurQueueChargeTime (j,i, charge_time_CTransN - 1, cctrans[j]);
