...
static void touch_Regcfg (void) {
    ...
    TKEY1->TRANS_CFG &= ~(3 << 13);
    TKEY1->TRANS_CFG |= TKEY_SwitchTime_8HCLK;
    TKEY_ChargeCurrentModeConfig (TKEY1, TKEY_ChargeCurrentMode1);
    TKEY_WorkModeConfig (TKEY1, TKEY_WorkMode_CurrentSourceCharge);
...
static void touch_Channelinit (void) {
    ...
    for (uint8_t i = 0; i < TKY_MAX_QUEUE_NUM; i++) {
        ...
        chx_mean = TKY_GetCurChannelMean (my_tky_ch_init[i].channelNum, my_tky_ch_init[i].chargeTime,
                                          my_tky_ch_init[i].disChargeTime, 10);
