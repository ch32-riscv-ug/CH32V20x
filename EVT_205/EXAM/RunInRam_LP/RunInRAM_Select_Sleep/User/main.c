 ...
 * @fn      FLASH_LP_Enter
 *
 * @brief   Enter FLASH Low power mode 1
 *
 * @return  none
 */
...
{
    uint32_t tmpreg = 0;
    /* FLASH low power mode FLASH_LP[1:0] = 10'b */
 ...
 * @fn      FLASH_LP_Exit
 *
 * @brief   Exit FLASH Low power mode 1
 *
 * @return  none
 */
__attribute__((section(".highcode")))
 ...
 *       tips:For the small package model of the chip, there are some pins that
 *         have not been led out compared to the largest package, or some pins
 *         that have been packaged but not used. These pins need to be set as 
 *         pull-down inputs to reduce current consumption. 
 *
 * @return  none
 */
void GPIO_ALL_IPD(void)
{
    ...
    RCC_PB2PeriphClockCmd(RCC_PB2Periph_GPIOA|RCC_PB2Periph_GPIOB|
            RCC_PB2Periph_GPIOC|RCC_PB2Periph_GPIOD|RCC_PB2Periph_GPIOE,ENABLE);
...
#if ((LP_Mode == LP_Stop_Mode)&&(Enter_LP_Mode == Enter_LP_WFE_Mode))
    SystemInit();
#endif
