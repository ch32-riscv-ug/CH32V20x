...
void EXTI4_IRQHandler(void) __attribute__((interrupt("WCH-Interrupt-fast"))); /* Interrupt handler function */
/*********************************************************************
 * @fn      EXTI4_IRQHandler
 *
 * @brief   The function EXTI4_IRQHandler handles the interrupt for EXTI lines 4  and performs certain
...
static void RxInit(void)
{
    ...
    NVIC_SetPriority(EXTI4_IRQn,0<<6);
    ...
    NVIC_SetPriority(TIM1_UP_IRQn,1<<6);
...
{
    GPIO_InitTypeDef GPIO_InitStructure={0};
    GPIO_PinAFConfig(SoftUARTHardware.txPort, GPIO_PinSource1, GPIO_AF1);
    ...
    TIM_OC4Init(SoftUARTHardware.txTIM, &TIM_OCInitstructure);

    TIM_OC4PreloadConfig(SoftUARTHardware.txTIM, TIM_OCPreload_Disable);
    ...
    DMA_InitStruct.DMA_PeripheralBaseAddr = (u32)&SoftUARTHardware.txTIM->CH4CVR;
...
static void TransmitBytes(vu8 *data, vu16 len)
{
    ...
    if (buffer != NULL)
    {
        for (u16 i = 0; i < (len > 0 ? len : length); i++)
        {
            ...
            while (DMA_GetFlagStatus(DMA1,DMA_FLAG_TC3) == RESET)
            ;
...
void Init(void)
{
    ...
    RCC_PB2PeriphClockCmd(RCC_PB2Periph_GPIOA | RCC_PB2Periph_GPIOB | RCC_PB2Periph_TIM1 | RCC_PB2Periph_AFIO , ENABLE);
...
void Init(u32 baudrate)
{
    ...
    SoftUARTHardware.txTIM = TIM3;
    SoftUARTHardware.txPort = GPIOB;
    SoftUARTHardware.txPin = GPIO_Pin_1;
    SoftUARTHardware.txDMA = DMA1_Channel3;
    // Configure the periph's para
