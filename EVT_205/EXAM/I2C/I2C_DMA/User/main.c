...
void IIC_Init(u32 bound, u16 address)
{
    ...
    // I2C1_SCL PB8(AF7)
    GPIO_PinAFConfig(GPIOB, GPIO_PinSource8, GPIO_AF7);
...
int main(void)
{
    ...
    for (j = 0; j < 5; j++)
    {
        ...
        while (DMA_GetFlagStatus(DMA1, DMA_FLAG_TC6) == RESET)
            ;

        DMA_Cmd(DMA1_Channel6, DISABLE);
        DMA_ClearFlag(DMA1, DMA_FLAG_TC6);
    ...
    for (t = 0; t < 5; t++)
    {
        ...
        while ((!DMA_GetFlagStatus(DMA1, DMA_FLAG_TC7)))
            ;
